...
/// <summary>
/// Polls Out/ for dropped XML files and forwards each one to the configured broker /message
/// endpoint, archiving the result to Sent/. Polling instead of FileSystemWatcher to avoid
/// picking up a file while it's still being written and to keep behaviour predictable across
/// platforms.
/// </summary>
    ...
    private async Task ProcessOutboxAsync(CancellationToken cancellationToken)
    {
        ...
        foreach (var filePath in Directory.EnumerateFiles(store.OutDirectory, "*.xml"))
        {
            ...
            try
            {
                ...
                var result = await client.SendAsync(payload, cancellationToken: cancellationToken);
                await store.SaveSentAsync(payload, messageIdentifier, result.Status, result.ResponseBody, cancellationToken);
