...
`InfrastructureOperatorConsumerCoordinator.HandleMessageAsync` (the handler passed to `StartPartitionAsync`) now relays each message to the operator's own SOAP endpoint via `IsbApiClient`, with the same authorize/retry/reachability-check/pause/dead-letter shape `EvuDeliveryCoordinator` uses for the opposite direction — see [[Business-Flow]] Flow 5.
